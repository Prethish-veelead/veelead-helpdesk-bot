    ...
    try:
        results = client.search(
            ...
            select=[
                ...
                "tags", "status", "author", "ai_citation_count",
                "helpful_count", "not_helpful_count", "source_ticket_id",
                "chunk_index", "total_chunks", "page", "pdf_url",
            ],
        )
    except HttpResponseError as e:
        log.error(f"Search failed: {e}")
...
# Categories rarely change. Cache the result for 60 seconds to avoid
# hitting Azure AI Search on every user request.
_categories_cache: Dict[str, Any] = {"value": None, "expires_at": 0.0}
_CATEGORIES_TTL_SEC = 60
...
def list_categories(only_published: bool = True) -> List[Dict[str, Any]]:
    ...
    Results are cached in-process for 60 seconds since categories rarely
    change. Saves an Azure Search facet query on every search request.
    """
    import time as _time
    cache_key = "pub" if only_published else "all"
    now = _time.time()
    ...
    _categories_cache[cache_key] = {
        "value": categories,
        "expires_at": now + _CATEGORIES_TTL_SEC,
    }
    return categories
