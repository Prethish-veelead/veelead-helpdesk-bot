...
# Log format — does NOT use %(request_id)s in the format string itself,
# because Azure SDK and other third-party libraries emit log records that
# don't go through our filter, and Python's formatter would crash with
# KeyError on those records.
#
# Instead, request_id is injected into YOUR log messages directly via the
# middleware (see _request_id_and_timing_middleware) — you'll see
# "[req-a3f2c901] → GET /search.json..." in the actual message text.
...
class _RequestIdFilter(logging.Filter):
    ...
    Kept for backward compatibility with code that explicitly reads
    record.request_id (e.g. custom Azure-side log shipping). Safe to
    keep even though we no longer use %(request_id)s in the format.
    """
    def filter(self, record: logging.LogRecord) -> bool:
        rid = _request_id_var.get()
        record.request_id = f"[req-{rid}] " if rid else ""
...
class _RequestIdFormatter(logging.Formatter):
    """Defensive formatter — guarantees no crash if record lacks request_id."""
...
# Apply defensive formatter to ALL handlers (including ones added later
# by libraries like Azure SDK or uvicorn).
...
async def _request_id_and_timing_middleware(request, call_next):
    ...
    try:
        ...
        if len(qs) > 120:
            qs = qs[:120] + "..."
        # request_id embedded directly in message — Azure SDK doesn't add it
        log.info(f"[req-{rid}] → {method} {path}{'?' + qs if qs else ''}")
    except Exception:
        pass  # logging must never break a request
    ...
    try:
        ...
        duration_ms = int((_t.time() - started) * 1000)
        # One-line summary: easy to grep for slow requests
        log.info(f"[req-{rid}] ← {status_code} in {duration_ms}ms")
        _request_id_var.reset(token)
...
def admin_usage(
    ...
    Example:
        ...
        GET /admin/usage?days=30  → last month
    """
    # Defensive: if the llm_usage table doesn't exist yet (e.g. fresh deploy
    # where init_db() hasn't run, or older cache.db without the new table),
    # try to create it first, then retry. Returns a helpful diagnostic
    # message instead of a generic HTTP 500.
    try:
        return cache.llm_usage_stats(days=days)
    except Exception as e:
        log.warning(f"admin_usage: first attempt failed: {e}")
        # Try to initialise tables and retry once
        try:
            cache.init_db()
            return cache.llm_usage_stats(days=days)
        except Exception as e2:
            log.error(f"admin_usage failed after init_db retry: {e2}")
            return {
                "error": "usage_stats_unavailable",
                "message": (
                    "Could not read LLM usage data. The llm_usage table may "
                    "not exist yet, or the cache database is unreachable. "
                    "Try restarting the App Service to run init_db() at startup."
                ),
                "exception_type": type(e2).__name__,
                "exception_detail": str(e2)[:200],
                "window_days": days,
                "total_calls": 0,
                "total_cost_inr": 0.0,
            }
