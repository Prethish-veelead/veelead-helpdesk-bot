...
def chunk_document(doc: Dict[str, Any], chunk_size: int = 400,
    ...
    Args:
        doc: dict with at least 'filename' and 'text' keys.
             May also have: 'sharepoint_file_id', 'article_title', 'category',
             'sub_category', 'tags', 'summary', 'status', 'doc_type', etc.
             OPTIONAL: 'pages' — list of (text, page_num) tuples. If present,
             chunks will be tagged with their source page.
        chunk_size: target words per chunk
        overlap: overlap words between chunks
    ...
    # Build list of (chunk_text, page_num) tuples
    chunks_with_pages: List[tuple] = []

    if pages_data:
        # Page-aware chunking: chunk each page separately so we know
        ...
        for page_text, page_num in pages_data:
            ...
            for c in page_chunks:
                chunks_with_pages.append((c, page_num))
    else:
        # Plain chunking — no page info
        plain_chunks = chunk_text(doc["text"], chunk_size=chunk_size, overlap=overlap)
        for c in plain_chunks:
            chunks_with_pages.append((c, None))
    ...
    for i, (chunk, page) in enumerate(chunks_with_pages):
        chunk_doc = {
            ...
            "page": page,                              # NEW: page number for PDFs
            "pdf_url": doc.get("pdf_url"),             # NEW: link to source file
        }
        output.append(chunk_doc)
