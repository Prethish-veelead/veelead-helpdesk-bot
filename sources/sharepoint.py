...
# # #         """Combine drive-item info + list-item metadata into a DocumentRef."""
# # #         filename = drive_item.get("name", "")
# # #         # SharePoint provides a direct webUrl that opens the file in browser
# # #         # (authenticated users can view; redirects to SharePoint web UI)
...
# # COLUMN_MAP = {
# #     "article_title": ["ArticleTitle", "Article_x0020_Title", "Title0"],
# #     # NOTE: Category and Sub-Category are LOOKUP fields — they only give us
# #     # an ID. We resolve the IDs to actual category names via HD_Categories
# #     # and HD_SubCategories lookup lists (see _resolve_lookups()).
# #     "category_lookup_id": ["KBCategoryLookupId"],
# #     "sub_category_lookup_id": ["KBSubCategoryLookupId"],
# #     "tags": ["Tags"],
# #     "summary": ["Summary"],
# #     # Status is actually called "ArticleStatus" in this library!
# #     "status": ["ArticleStatus", "Status"],
# #     "approved_by": ["ApprovedByLookupId", "Approved_x0020_By", "ApprovedBy"],
# #     "author": ["Author0LookupId", "AuthorLookupId"],
# #     "publish_date": ["PublishDate", "Publish_x0020_Date"],
# #     "view_count": ["ViewCount", "View_x0020_Count"],
# #     "helpful_count": ["HelpfulCount", "Helpful_x0020_Count"],
# #     "not_helpful_count": ["NotHelpfulCount", "Not_x0020_Helpful_x0020_Count"],
# #     "ai_citation_count": ["AICitationCount", "AI_x0020_Citation_x0020_Count"],
# #     "source_ticket_id": ["SourceTicketID", "Source_x0020_TicketID"],
# #     "article_content": ["ArticleContent", "Article_x0020_Content"],
# #     "last_indexed": ["LastIndexed", "Last_x0020_Indexed"],
# # }
...
# #         self._drive_id: Optional[str] = None
# #         self._list_id: Optional[str] = None
# #         # Lookup caches: ID → display name
# #         self._category_lookup: Dict[str, str] = {}
# #         self._subcategory_lookup: Dict[str, str] = {}
...
# #         # Resolve category + subcategory lookup tables
# #         self._resolve_lookups(lists.get("value", []))

# #     def _resolve_lookups(self, all_lists: list) -> None:
# #         """
# #         Fetch the HD_Categories and HD_SubCategories lookup tables.

# #         SharePoint stores Category and Sub-Category as LOOKUP fields — the
# #         document only has an integer ID (e.g. KBCategoryLookupId='1'). The
# #         actual name ("IT", "HR", ...) lives in a separate list.

# #         We fetch those lists once and build {id → name} dicts. Cached for
# #         the session — refreshed when ids are re-resolved.
# #         """
# #         if self._lookups_loaded:
# #             return

# #         # Find HD_Categories and HD_SubCategories list IDs
# #         cat_list_id = None
# #         subcat_list_id = None
# #         for lst in all_lists:
# #             display = (lst.get("displayName") or "").lower()
# #             if display in ("hd_categories", "kbcategories", "categories"):
# #                 cat_list_id = lst["id"]
# #             elif display in ("hd_subcategories", "kbsubcategories", "subcategories"):
# #                 subcat_list_id = lst["id"]

# #         def _fetch_lookup_table(list_id: str) -> Dict[str, str]:
# #             """Returns {id_str: title_or_name}."""
# #             if not list_id:
# #                 return {}
# #             mapping: Dict[str, str] = {}
# #             url = f"{GRAPH_BASE}/sites/{self._site_id}/lists/{list_id}/items?expand=fields&$top=200"
# #             while url:
# #                 data = self._http_get(url).json()
# #                 for item in data.get("value", []):
# #                     fields = item.get("fields") or {}
...
# #                     name = (
# #                         fields.get("Title")
# #                         or fields.get("CategoryName")
# #                         or fields.get("Name")
# #                         or ""
# #                     )
# #                     item_id = str(fields.get("id") or item.get("id") or "")
# #                     if item_id and name:
...
# #         try:
# #             self._category_lookup = _fetch_lookup_table(cat_list_id)
# #             self._subcategory_lookup = _fetch_lookup_table(subcat_list_id)
# #             log.info(
# #                 f"Loaded {len(self._category_lookup)} categories, "
# #                 f"{len(self._subcategory_lookup)} sub-categories"
# #             )
# #             if self._category_lookup:
# #                 log.info(f"  Categories: {list(self._category_lookup.values())}")
# #         except Exception as e:
# #             log.warning(f"Could not load lookup tables: {e}")
...
# #         # Resolve Category and Sub-Category from lookup IDs
# #         cat_id = _pick_field(list_meta, COLUMN_MAP["category_lookup_id"])
# #         sub_cat_id = _pick_field(list_meta, COLUMN_MAP["sub_category_lookup_id"])
# #         category_name = (
# #             self._category_lookup.get(str(cat_id)) if cat_id else None
# #         ) or "Uncategorized"
# #         sub_category_name = (
# #             self._subcategory_lookup.get(str(sub_cat_id)) if sub_cat_id else None
# #         )
...
# #             author=(
# #                 _pick_field(list_meta, COLUMN_MAP["approved_by"])
# #                 or _pick_field(list_meta, COLUMN_MAP["author"])
# #                 or (drive_item.get("createdBy", {}).get("user", {}) or {}).get("displayName")
# #             ),
# #             publish_date=_parse_iso(_pick_field(list_meta, COLUMN_MAP["publish_date"])),
# #             view_count=int(float(_pick_field(list_meta, COLUMN_MAP["view_count"]) or 0)),
# #             helpful_count=int(float(_pick_field(list_meta, COLUMN_MAP["helpful_count"]) or 0)),
# #             not_helpful_count=int(
# #                 float(_pick_field(list_meta, COLUMN_MAP["not_helpful_count"]) or 0)
# #             ),
# #             ai_citation_count=int(
# #                 float(_pick_field(list_meta, COLUMN_MAP["ai_citation_count"]) or 0)
# #             ),
# #             source_ticket_id=_pick_field(list_meta, COLUMN_MAP["source_ticket_id"]),
# #             article_content=_pick_field(list_meta, COLUMN_MAP["article_content"]),
# #         )
...
#         meta_by_name = self._fetch_list_metadata()
#         # Cache it so subsequent calls in this sync don't re-fetch
#         self._list_meta_cache = meta_by_name
...
#         """
#         self._resolve_ids()
#         # Force fresh metadata bulk-fetch on each sync
#         self._invalidate_meta_cache()
...
#     def _fetch_list_metadata_for_item(self, filename: str) -> dict:
#         """
#         Fetch metadata for a single item by filename.

#         Uses a session-cached bulk fetch (NOT per-file filter), because
#         filtering by FileLeafRef requires SharePoint indexing on that
#         field which we can't guarantee. Bulk fetch is also much faster
#         for delta syncs with multiple changed files.
#         """
#         if not self._list_id:
#             return {}

#         # Use cached metadata if available, else fetch full list once
#         if not hasattr(self, "_list_meta_cache") or self._list_meta_cache is None:
#             try:
#                 self._list_meta_cache = self._fetch_list_metadata()
#             except Exception as e:
#                 log.warning(f"Could not bulk-fetch list metadata: {e}")
#                 self._list_meta_cache = {}

#         return self._list_meta_cache.get(filename, {})

#     def _invalidate_meta_cache(self) -> None:
#         """Call before each sync to force a fresh metadata fetch."""
#         self._list_meta_cache = None
...
class SharePointSource(DocumentSource):
    ...
    def _resolve_lookups(self, all_lists: list) -> None:
        ...
        def _fetch_lookup_table(list_id: str) -> Dict[str, str]:
            ...
            Field priority (matches the actual SharePoint schema for
            HD_Categories / HD_SubCategories):
              1. SubCategoryName  — used in HD_SubCategories
              2. CategoryName     — used in HD_Categories
              3. Title            — generic SharePoint default
              4. Name             — fallback
            """
            if not list_id:
                return {}
        ...
        try:
            ...
            if self._category_lookup:
                log.info(f"  Categories:     {list(self._category_lookup.values())}")
            if self._subcategory_lookup:
                log.info(f"  Sub-categories: {list(self._subcategory_lookup.values())}")
        except Exception as e:
            log.warning(f"Could not load lookup tables: {e}")
